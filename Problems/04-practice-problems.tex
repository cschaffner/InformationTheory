...
\begin{exercise}[Bernoulli typical sets]
	...
	\end{subex}
	\begin{subex}
		Prove that for any $\epsilon > 0$, any $p,q$ such that $p + 2\varepsilon < q \leq 1/2$, and big enough $n$, it holds that $|A_{\varepsilon}^{(n)}(p)| < |A_{\varepsilon}^{(n)}(q)|$.
	\end{subex}
\begin{subex}
	Prove that for any $\epsilon > 0$, any $p,q$ such that $p + 2\varepsilon < q \leq 1/2$, and big enough $n$, it holds that $A_{\varepsilon}^{(n)}(p) \cap A_{\varepsilon}^{(n)}(q) = \emptyset$.
\end{subex}
\end{exercise}
