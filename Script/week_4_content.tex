...
A time-invariant Markov process is finite-state if $|\mathcal{X}|$ is finite. \yfke{Or should we say: if the set of elements $\mathcal{X}$ that have non-zero probability is finite?} \chris{Hmm, do you mean at any time step $i$? And do you have a good reason for being this restrictive? I think we should simply go for $\mathcal{X}$ as \href{https://en.wikipedia.org/wiki/State_space}{state space} of the Markov chain.}
\end{definition}
