...
We start by studying a restricted form of dependence between variables: Markov chains. Random variables forms a Markov chain if the distribution of each random variable depends only on the outcome of the random variable that directly precedes it.

\begin{definition}[Markov chain (length 3)]\label{def:Markov-chain}
The random variables $X, Y,$ and $Z$ form a Markov chain (notation: $X \to Y \to Z$) if
\[
P_{Z|XY} = P_{Z|Y}.
\]
\end{definition}
...
\begin{definition}[Markov chain (length $n$)]
...
This process is stationary, which can be shown by induction on $n$. For $n = 1$, it is immediate from the definition of the process: $P_{X_1} = P_X = P_{X_{1+k}}$ for all $k \in \mathbb{N}$. For the induction step, suppose that for some $n \in \mathbb{N}$, for all $k$, $P_{X_1 \cdots X_n} = P_{X_{1+k} \cdots X_{n+k}}$. Then also for all $k$,
\begin{align*}
...
Throw a fair coin 10 times: this gives the finite sample space $\{\texttt{H},\texttt{T}\}^{10}$. Define the stochastic process $\{X_i\}$ by setting $X_i = 0$ if coin number $(i \mod\ 10)$ is heads, and $X_i = 1$ otherwise.

This process is stationary. You can prove this yourself by observing that for all $i$, $X_i = X_{i+10} = X_{i+20} = \dots$, and using a similar induction argument as in Example~\ref{ex:stationary-1}.
\end{example}
...
The entropy rate $H(\mathcal{X})$ of a stochastic process $\{X_i\}$ is
\[
...
\begin{example}
Consider a process $\{X_i\}$ where the $X_i$ are i.i.d. fair coin tosses. Then
\begin{align*}
H(\mathcal{X}) &= \lim_{n \to \infty} \frac{1}{n} H(X_1, \dots X_n)\\
&= \lim_{n \to \infty} \frac{1}{n} H(X_1) \cdot H(X_1) \cdots H(X_n)\\
&= \lim_{n \to \infty} \frac{n}{n} H(X_1)\\
&= 1.
\end{align*}
So, every new coin toss increases the entropy of the entire observed sequence by 1.
\end{example}
...
If $\lim_{n \to \infty} a_n = a$ and $b_n = \frac{1}{n} \sum_{i=1}^n a_i$, then $\lim_{n \to \infty} b_n = a$.
\end{theorem}
