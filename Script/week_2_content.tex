...
\yfke{I'm not sure what happens to $\delta'$ here: the proof should work for any $\delta' > 0$, aren't we restricting the freedom here? Or can we always find an epsilon to accommodate for the values of $\delta$ and $\delta'$ that are given?} \chris{we have to choose $n$ large enough in order to make $\delta'$ as small as we want.}
\end{proof}
