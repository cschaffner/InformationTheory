...
\end{align*}
\end{example}
As stated, the channel capacity reflects the maximum amount of information that \emph{could} be sent over a noisy channel. The question remains whether this capacity is \term{achievable} by some code in the following sense:
...
The rest of this chapter will be concerned with proving the following theorem, due to Shannon:

\begin{theorem}[Shannon's noisy-channel coding theorem]\label{thm:ncc}
...
\begin{proof}[Proof of Theorem~\ref{thm:ncc} (forward direction)]
...
%\vspace{5cm}
%We can simplify the expression for $P[\error]$ using the fact that 
%
%With this decoding procedure in mind, we will analyze the error probability (see Definition~\ref{}) for the code \cran,
%\begin{align}
%\lambda_w(\cran) = P[\hat{w} \neq w \mid \cran(w) \text{ was sent over the channel}].
%\end{align}
%Consider the average error probability, where the expectation is taken over a uniformly random word $w$ and the code \cran, randomly constructed as described above:
%\begin{align}
%P[\error] &= \sum_{\cran} P[C] \cdot \left(\sum_{w=1}^{2^{n\cdot R}} P[w] \cdot \lambda_w(\cran) \right) \nonumber\\
%&= \sum_{\cran} \frac{1}{2^{n \cdot R}} \cdot
%\end{align}
%
%\vspace{5cm}
%
%For any particular code $\mathcal{C} = \{x(1), ..., x(2^{n \cdot R})\}$, the probability of sampling exactly that code is
%\begin{align}
