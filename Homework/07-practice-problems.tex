...
[CT, 7.31] We wish to encode a Bernoulli($\alpha$) process $V_1, V_2, ...$ for transmission over a binary symmetric channel with crossover probability $\epsilon$.
\begin{center}
\begin{tikzpicture}
\node at (0,0) {$V^n$};
\node at (2,0) {$X^n(V^n)$};
...
[CT, 7.36] Consider the discrete memoryless channel $(\mathcal{X}, P_{Y|X}, \mathcal{Y})$ with $\mathcal{X} = \{-1,1\}$, and $Y = ZX$ for a random variable $Z$ with $\mathcal{Z} = \{-1,1\}$.
\begin{subex}
...
If you have solved all of the above problems before the exercise session is over, take the rest of the time to review previous exercise sets, homeworks or concept quizzes, or to ask any questions that you might have.
\end{exercise}
